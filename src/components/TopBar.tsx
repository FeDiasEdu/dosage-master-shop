...
export default function TopBar({ activeTab, onTabChange, user, onAuthClick }: TopBarProps) {
  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-[200] h-14 border-b border-border bg-card/92 backdrop-blur-[14px] transition-colors">
        <div className="h-full max-w-screen-2xl mx-auto grid grid-cols-[1fr_auto_1fr] items-center px-6">
          ...
          <div className="flex items-center h-full gap-3">
            <a href="#" className="flex items-center gap-2.5 no-underline" onClick={(e) => { e.preventDefault(); onTabChange("guia"); }}>
          ...
          <div className="flex items-center justify-end gap-2">
            <button
              onClick={onAuthClick}
              className="inline-flex items-center gap-1.5 px-3 h-9 rounded-full border border-border bg-card text-xs font-medium cursor-pointer transition-all hover:border-foreground hover:bg-secondary"
            >
              {user ? (
      ...
      {/* Mobile bottom nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-[200] sm:hidden border-t border-border bg-card/95 backdrop-blur-[14px] safe-area-bottom">
